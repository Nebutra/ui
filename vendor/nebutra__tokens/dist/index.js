...
var THEME_COOKIE_MAX_AGE = 365 * 24 * 60 * 60;
function writeThemeCookie(resolved, cookieName) {
  if (typeof document === "undefined") return;
  document.cookie = `${cookieName}=${resolved}; Max-Age=${THEME_COOKIE_MAX_AGE}; Path=/; SameSite=Lax`;
}
function applyThemeToDom(resolved, attribute, disableTransitionOnChange, nonce) {
  if (typeof document === "undefined") return;
...
function ThemeProvider({
  ...
  const resolvedTheme = effectiveTheme === "system" ? enableSystem ? systemTheme : "light" : effectiveTheme;
  const setTheme = useCallback(
    (next) => {
      if (forcedTheme !== void 0) return;
      setThemeState(next);
      try {
        if (next === "system") {
...
function useBrand(options = {}) {
  ...
  const [brand, setBrand] = useState2(null);
  const [brandId, setBrandId] = useState2(null);
  useEffect2(() => {
...
function useBrandIframePreview(options = {}) {
  ...
  const writePreviewDocument = useCallback2((next, bodyHtml = "") => {
